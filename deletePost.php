...
<?php 
	...
	while($data_rows = $stmtUpdate->fetch()) {
		$deleteTitle = $data_rows['name'];
		$deleteCategory = $data_rows['category'];
		$deleteImage = $data_rows['image'];
		$deleteContent = $data_rows['content'];
	}
	...
	if(isset($_POST["Submit"])){
			...
			//Check for successfull deletion to DB
			if($execute){
				//Deleting image of particular content
				$PathDeleteImage = "upload/$deleteImage";
				unlink($PathDeleteImage);
...
<html>
	<head>
		...
		<title>Sports News | Delete Post</title>
		<meta charset="utf-8">
	...
	<body>
		...
		<header class="py-2">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<h1 class="text-center">Post Deletion Suite</h1>
					</div>
				</div>
			</div>
		</header>
		...
		<section class="container py-2 mb-2">
			<div class="row">
				<div class="offset-lg-1 col-lg-10">
					...
					<form class="" action="deletePost.php?id=<?php echo $searchParam; ?>" method="post" enctype="multipart/form-data">
						<div class="card bg-light text-dark">
							...
							<div class="card-body bg-light text-dark">
								<div class="form-group">
									...
									<input disabled class="form-control" type="text" name="postName" id="title" placeholder="Insert post title here." value="<?php echo $deleteTitle; ?>">
								</div>
								...
								<div class="form-group">
									...
									<input type="file" name="image" id="imageSelect" value="<?php echo $deleteImage; ?>">
								</div>
								<!-- Post content -->
								<div class="form-group">
									<label for="postContent"><span class="fieldInfo">Post Content: </span></label>
									<textarea disabled class="form-control" id="postContent" name="postContentArea" rows="5" cols="50"><?php echo $deleteContent; ?>
									</textarea>
								</div>
								<!-- Button to take back to dashboard -->
								<div class="row">
									<div class="col-lg-6 mb-2">
										<a href="Dashboard.php" class="btn btn-warning btn-block"><i class="fas fa-arrow-alt-circle-left"></i> Back to Dashboard</a>
									</div>
									<div class="col-lg-6">
										<button type="submit" value="Save" name="Submit" class="btn btn-block btn-danger">Delete Post <i class="fas fa-trash-alt"></i></button>
									</div>
								</div>
							</div>
						</div>
